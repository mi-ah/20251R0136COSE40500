				    ...
				    struct iov_iter *from)
{
	 ...
	 */
	if (!iocb || !file || !inode || !from){
		printk(KERN_ERR "[BTRFS WRITE] Null pointer detected, aborting log\n");
		return -EINVAL;
	}

	tmp_path = (char *)__get_free_page(GFP_KERNEL);
	if (!tmp_path){
		printk(KERN_ERR "[BTRFS WRITE] Failed to allocate memory for path\n");
		return -ENOMEM;
	}

	d_path_result = d_path(&iocb->ki_filp->f_path, tmp_path, PAGE_SIZE);
	if (IS_ERR(d_path_result)){
		printk(KERN_ERR "[BTRFS WRITE] Failed to get path, error=%ld\n", PTR_ERR
		(d_path_result));
		free_page((unsigned long)tmp_path);
		return PTR_ERR(d_path_result);
	}

	if (strstr(d_path_result, "/home/spaisya/btrfs_dir")){
		printk(KERN_INFO "[BTRFS WRITE] file=%s, inode=%lu, offset=%lld\n",
			d_path_result, file_inode(file)->i_ino, offset);
	}
	free_page((unsigned long)tmp_path);
